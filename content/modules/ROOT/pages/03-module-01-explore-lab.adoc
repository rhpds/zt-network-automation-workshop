...
* link:https://www.redhat.com/en/technologies/management/ansible/automation-execution-environments[Execution Environments^, window=_blank] -- container images that can be utilized as Ansible execution.
...
In Visual Studio Code, under the *File* menu, choose *Open Folder...* and select the `network-workshop` directory to open the File Explorer:
...
TIP: Depending on your screen resolution, it's likely that the Terminal menu is hidden under the 3 dots `...` located to the right of the other menu options, as seen in the above screenshot.

Your new terminal should already be in the right place. If it is, you'll see `network-workshop` as part of the prompt. If it isn't, navigate to the `network-workshop` directory in the VS Code terminal.
...
TIP: The Showroom environment has a copy icon to the left of the "BASH" designation, which you can use for any commands you are asked to run in a terminal.
...
Press the *ESC* key to back out of any interactive `ansible-navigator` screen. Pressing *ESC* on the Welcome screen will return you to your terminal prompt, or you can type `:quit` on any screen.
...
Run the command to bring up inventory in the TUI:
